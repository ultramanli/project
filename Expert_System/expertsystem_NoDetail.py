    ...
    while(1):#没得出结果之前的大循环
        ...
        for rule in rules:
            ...
            if infeature==False:
                ...
                continue
            ##############################################################
            feature.insert(0,rear[ruleid])#后件进入特征集（构建完冲突集后前件进入dropin且从特征集中删除前件）
            dropin=[rear[ruleid],[]]#drop=[dropin,dropin,dropin] dropin=[当前在特征集中要被替换的后件,[后件集]]
            #########冲突集构建###############################
            idin=ruleid#用来遍历规则集且不影响当前规则id      
            ...
            drop.append(dropin)#dropin入栈 
            ###########################################

            for i in rule:#特征集中删除前件
...
while(1):
    ...
    print('\n')
    if(a=='q'):break
    feature.append(a)#构建特征集
show=feature.copy()#呈现最初所选特征
if(len(feature)<4):
    waring='---输入特征较少，结果可能有偏差，仅供参考---'
else:
    waring='----结果仅供参考----'
    
result=reasoning()#调用推理函数推理
#######输出结果###################################
if(result!=0):
     db = pymysql.connect("localhost","root","root","test",charset='utf8' )  
     cursor = db.cursor()# 使用cursor()方法获取操作游标
